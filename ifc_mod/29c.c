 ...
 *
 * 29c.c
 * Interface module for 29C... memories
 *
 */
...
const char fw_signature[FW_SIGNATURE_SIZE] = "29c";
...
#define SET_ADDRESS(a) do{                                             \
  hiaddr = a >> 9;                                                     \
  update_hiaddr();                                                     \
  GPIFADRH = (a >> 8) & 0x01;  SYNCDELAY;                              \
  GPIFADRL = a & 0xFF;         SYNCDELAY;                              \
}while(0)

static void memory_command_sequence (BYTE cmd)
{
  SET_ADDRESS(ADDR_UNLOCK1);
  GPIFSGLDATLX = CMD_UNLOCK1;
  WAIT_FOR_GPIF_DONE();
  SET_ADDRESS(ADDR_UNLOCK2);
  GPIFSGLDATLX = CMD_UNLOCK2;
  WAIT_FOR_GPIF_DONE();
  SET_ADDRESS(ADDR_CMD);
  GPIFSGLDATLX = cmd;
  WAIT_FOR_GPIF_DONE();
}
...
BOOL ifc_erase_chip(void)
{
  ...
  memory_command_sequence(CMD_ERASE);
  memory_command_sequence(CMD_CHIP_ERASE);
